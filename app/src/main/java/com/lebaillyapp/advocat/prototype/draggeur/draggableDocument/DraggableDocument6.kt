...
fun DraggableDocument6(
    ...
    // -------------------------------------------------------------------------
    // SEUL CHANGEMENT VS V5 : CACHE DES RENDER EFFECTS
    //
    // Dans la V5, RenderEffect.createBlurEffect() était instancié à chaque
    // recomposition dans les blocs graphicsLayer — soit potentiellement à chaque
    // frame de gesture. C'est une allocation d'objet inutile quand la valeur
    // n'a pas changé de façon perceptible.
    //
    // Fix : on quantifie la valeur continue en paliers discrets et on wrap dans
    // remember(key) — l'objet RenderEffect n'est recréé que si la valeur franchit
    // un palier, pas à chaque frame.
    //
    // Paliers ombre  : 0.05f sur lift (0..1)  → 20 paliers max
    // Paliers blur   : 0.5f  sur blur (0..4)  → 8  paliers max
    //
    // Tout le reste du code est strictement identique à la V5.
    // -------------------------------------------------------------------------
        ...
        Box(
            modifier = Modifier
                ...
                .graphicsLayer {
                    ...
                    // Remplace l'instanciation inline — objet caché, pas recréé à chaque frame
        ...
        Box(
            modifier = Modifier
                .graphicsLayer {
                    ...
                    // Remplace le if + instanciation inline — null si sous le seuil
